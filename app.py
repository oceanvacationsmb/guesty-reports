...
.summary-card{background:#f9f5f9;border:1px solid #ddd;padding:12px;text-align:center;border-radius:5px}
...
<body>
<div class="sidebar">
<h3>🎯 OCEAN VACATIONS</h3>
<div id="tokenStatus" class="token-status token-inactive">🔴 LOADING...</div>
...
<button class="btn btn-primary" onclick="generateStatement()">▶ STATEMENT</button>
<button class="btn btn-success" onclick="openAddExpense()" style="font-size:13px;font-weight:bold">💰 ADD EXPENSE</button>
<button class="btn btn-success" onclick="generateSummary()">📊 SUMMARY REPORT</button>
<div class="settings-header">📊 REPORTS</div>
<button class="btn btn-info" onclick="showIncomeReport()">💰 INCOME REPORT</button>
<button class="btn btn-info" onclick="showHistory()">🕒 HISTORY</button>
<button class="btn btn-info" onclick="show1099()">🕒 1099</button>
<div class="settings-header">⚙️ SETTINGS</div>
<button class="btn btn-info" onclick="openOwners()" style="font-size:11px">⚙️ OWNER SETTINGS</button>
<button class="btn btn-info" onclick="openTax()" style="font-size:11px">⚙️ TAX SETTINGS</button>
<button class="btn btn-info" onclick="openVendorSettings()" style="font-size:11px">🏢 VENDORS</button>
<hr style="border:1px solid #555;margin:20px 0">
<button class="btn btn-warning" onclick="setupToken()" style="font-size:11px">🔑 GITHUB SETUP</button>
<button class="btn btn-warning" onclick="setupDropbox()" style="font-size:11px">📁 DROPBOX SETUP</button>
</div>
...
<div>
<div class="modal-title">⚙️ OWNER MANAGEMENT</div>
...
<div class="modal-field"><label class="modal-label">🔗 GUESTY REPORT URL</label><input type="text" id="ownerGuestyUrl" class="modal-input" placeholder="https://guesty.com/..."></div>
<button class="btn btn-success" onclick="addOwner()" style="width:100%;margin-bottom:20px">➕ ADD OWNER</button>
<div id="ownersList" style="max-height:350px;overflow-y:auto;margin-bottom:20px"></div>
<div class="modal-actions"><button class="btn-save" onclick="saveOwnerData()">💾 SAVE ALL</button><button class="btn-cancel" onclick="closeAll()">✗ CLOSE</button></div>
</div>
</div>

<div id="taxModal" class="modal">
<div>
<div class="modal-title">⚙️ TAX SETTINGS</div>
<div id="taxContent" style="max-height:400px;overflow-y:auto;margin-bottom:20px"></div>
<div class="modal-actions"><button class="btn-save" onclick="saveTaxData()">💾 SAVE ALL</button><button class="btn-cancel" onclick="closeAll()">✗ CLOSE</button></div>
</div>
</div>

<div id="vendorSettingsModal" class="modal">
<div>
<div class="modal-title">🏢 VENDOR MANAGEMENT</div>
...
<button class="btn btn-success" onclick="addNewVendor()" style="width:100%;margin-bottom:20px">➕ ADD VENDOR</button>
<div id="vendorSettingsList" style="max-height:350px;overflow-y:auto;margin-bottom:20px"></div>
<div class="modal-actions"><button class="btn-save" onclick="saveVendorData()">💾 SAVE ALL</button><button class="btn-cancel" onclick="closeAll()">✗ CLOSE</button></div>
</div>
</div>
...
<div id="expenseCheckmark" class="success-checkmark" style="display:none">✓</div>
...
<div class="modal-actions"><button class="btn-add" onclick="submitExpense()">✓ ADD EXPENSE</button><button class="btn-cancel" onclick="doneAddingExpenses()">✓ DONE</button></div>
...
// NEW: read linen fee from possible column names (keeps code resilient)
function linenFee(row){
  return (
    num(row["LINEN FEE"]) ||
    num(row["LINENS FEE"]) ||
    num(row["LINEN"]) ||
...
function updateTokenStatus(){
  ...
  if(githubToken){
    status.textContent="🟢 TOKEN ACTIVE";
...
function showIncomeReport(){
  ...
  html += "<div style='font-size:12px;color:#666;margin-bottom:20px'>";
  html += "On this page you can run report for single, multiple or all properties.";
  html += "</div>";

  html += "<div class='summary-card' style='margin-bottom:20px'>";
  html += "<div class='summary-label'>GROSS INCOME REVENUE</div>";
  html += "<div class='summary-value'>"+money(masterTotals.acc)+"</div>";
  html += "</div>";

  html += "<div style='margin-bottom:20px'>";
  html += "<label style='font-weight:bold'>REPORT TYPE</label>";
  html += "<select id='incomeType' style='width:200px;margin-left:10px'>";
  html += "<option value='full'>FULL REPORT</option>";
  html += "<option value='gri'>GRI REPORT</option>";
  html += "</select>";
  html += "<button style='margin-left:15px;padding:6px 12px;background:#000;color:#fff;border:none;border-radius:4px' onclick='runIncomeReport()'>RUN</button>";
  html += "</div>";
...
function openOwners(){
  ...
  Object.keys(OWNERS).sort().forEach(name=>{
    ...
    html+="</select>";
    html+="<label class='modal-label'>🔗 GUESTY REPORT URL</label>";
    ...
    html+="<button class='btn btn-danger' onclick=\"deleteOwner('"+name+"')\" style='width:100%;margin-top:10px;padding:8px'>🗑️ DELETE OWNER</button>";
    html+="</div>";
  });
  list.innerHTML=html;
      ...
      .then(({response,sha})=>{
        if(response.status===409){
          if(retryCount<3){
            ...
            alert("❌ Could not save after 4 attempts");
          }
          return;
        }
...
function setupToken(){
  ...
  fetch("https://api.github.com/user",{headers:{"Authorization":"token "+githubToken}})
    ...
    .then(u=>{
      ...
      alert("✓ Token verified and saved locally!");
      updateTokenStatus();
      loadData();
    })
    .catch(e=>{
...
function displayStatement(){
  const t=OWNERS[currentOwner];
  const type=t.type;
  let html="<div class='action-buttons'><button class='copy-link-btn' onclick='generateLink()'>🔗 GENERATE & COPY LINK</button></div>";
  ...
  Object.keys(propertyTotals).forEach(prop=>{
    ...
    html+="<div class='property-section'><div class='property-title'><span class='property-icon'>🏠</span>"+prop.toUpperCase()+"</div>";
...
function generateLink(){
  alert("✓ Dropbox feature coming soon!");
}
...
function generateSummary(){
  ...
  let html="<div class='action-buttons'><button class='back-btn' onclick='generateStatement()'>← BACK</button><button class='print-btn' onclick='window.print()'>🖨️ PRINT</button></div>";
  ...
  if(type==="draft"){
    ...
    html+="<div style='margin:20px auto;padding:15px;border:2px solid #fbc02d;background:#fff9c4;border-radius:5px;text-align:center'><div style='font-size:14px;font-weight:bold;color:#f57f17;margin-bottom:10px;text-transform:uppercase'>💳 SALES COMMISSION DUE</div><div style='text-align:left;color:#333'>";
    html+="<div style='display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #ddd'><span>ACCOMMODATION</span><span>"+money(masterTotals.acc)+"</span></div>";
    ...
    html+="<div style='margin:20px auto;padding:15px;border:2px solid #4caf50;background:#e8f5e9;border-radius:5px;text-align:center'><div style='font-size:14px;font-weight:bold;color:#2e7d32;margin-bottom:10px;text-transform:uppercase'>💰 PAYOUT SUMMARY</div><div style='text-align:left;color:#333'>";
    ...
    html+="<div style='margin:20px auto;padding:15px;border:2px solid #fbc02d;background:#fff9c4;border-radius:5px;text-align:center'><div style='font-size:14px;font-weight:bold;color:#f57f17;margin-bottom:10px;text-transform:uppercase'>💳 SALES COMMISSION</div><div style='text-align:left;color:#333'>";
    html+="<div style='display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #ddd'><span>ACCOMMODATION</span><span>"+money(masterTotals.acc)+"</span></div>";
    html+="<div style='display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #ddd'><span>PMC COLLECTED</span><span>"+money(masterTotals.pmc)+"</span></div>";
    html+="<div style='display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #ddd'><span>COMMISSION FEE ("+t.salesFeePercent+"%)</span><span>"+money(salesFee)+"</span></div>";
    html+="<div style='display:flex;justify-content:space-between;padding:12px;background:#000;color:#fff;font-weight:bold'><span>COMMISSION DUE</span><span>"+money(salesFee)+"</span></div></div></div>";
