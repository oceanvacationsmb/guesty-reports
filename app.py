...
# --- 2. INTERNAL DATABASE INITIALIZATION ---
...
# --- 3. DASHBOARD UI ---
st.set_page_config(page_title="Owner Rule Manager", layout="wide")
...
with st.sidebar:
    ...
    # SECTION: MANAGE OWNER RULES
    st.header("⚙️ Manage Owner Rules")
    
    # 1. Choose which owner to edit
    edit_list = list(st.session_state.owner_db.keys())
    target_owner = st.selectbox("Select Owner to Edit/Add", ["+ Add New"] + edit_list)
    
    # 2. Setup fields based on selection
    ...
    # 3. Adjustment Fields
    new_pct = st.number_input(f"Commission % for {name_input}", 0.0, 100.0, float(current_pct))
    new_type = st.selectbox(f"Payment Style", ["Draft", "Payout"], index=0 if current_type == "Draft" else 1)
    ...
    st.divider()
    # SECTION: REPORT SELECTOR
    st.header("📊 View Report")
    active_owner = st.selectbox("Switch Active Report", sorted(st.session_state.owner_db.keys()))

# --- 4. CALCULATIONS ---
conf = st.session_state.owner_db[active_owner]

# Mock data for demonstration - will use Guesty data if API connects
raw_data = [
...
with c4:
    if conf['type'] == "Draft":
        total_draft = total_comm + total_clean + (gross * (conf.get('web_fee', 0)/100))
...
st.download_button("📥 Download CSV", data=csv, file_name=f"{active_owner}_Settlement.csv")
