...
function processData(){
  ...
  const t=OWNERS[currentOwner];
  propertyTotals={};
  masterTotals={gross:0,acc:0,clean:0,pmc:0,expenses:0,draft:0,owner:0,websiteFee:0,vrboFee:0,totalTaxCollected:0};
  taxByProperty={};
  let byProp={};
  ...
  Object.keys(byProp).forEach(prop=>{
    let gross=0,acc=0,clean=0,pmc=0,websiteFee=0,vrboFee=0,totalTax=0;
    ...
    propertyTotals[prop]={gross:gross,acc:acc,clean:clean,pmc:pmc,websiteFee:websiteFee,vrboFee:vrboFee,expenses:exp,draft:draft,owner:owner,tax:totalTax,reservations:byProp[prop]};
  ...
  Object.keys(propertyTotals).forEach(prop=>{
    ...
    if(type==="draft"){
      ...
      p.reservations.forEach(row=>{
        ...
        // PMC calculated on accommodation
